// Ported unchanged from services/glassParser.js — no Node-specific APIs, pure string/regex logic.

export function detectLayers(text) {
  const matches = text.match(/\d+\+\d+(\+\d+)*/)
  if (!matches) return 1
  return matches[0].split('+').length
}

export function extractThicknesses(text) {
  // Detect formats like: 5+5, 6+6, 12+12, 8+8+8
  const layered = text.match(/(\d+)(\+\d+)+/)
  if (layered) {
    return layered[0].split('+').map(v => parseInt(v))
  }
...
export function extractDimensions(text) {
  // Supports: 1000x2000, 1000 x 2000, 1ft x 1ft
...
export function extractPieces(text) {
